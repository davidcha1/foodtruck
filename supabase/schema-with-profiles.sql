...
-- Users table (core auth info only)
...
CREATE TABLE IF NOT EXISTS public.listings (
    ...
    space_size_sqm INTEGER, -- space size in square meters
    max_trucks INTEGER DEFAULT 1, -- how many trucks can fit
    images TEXT[] DEFAULT '{}',
...
CREATE TABLE IF NOT EXISTS public.bookings (
    ...
    special_requests TEXT, -- vendor's special requirements for this booking
    venue_owner_notes TEXT, -- venue owner's notes about the booking
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
...
-- Function to create profile based on user role
...
BEGIN
    ...
    ELSIF NEW.role = 'vendor' THEN
        ...
        VALUES (NEW.id, 'My Food Truck'); -- Default name, user can update
    END IF;
    RETURN NEW;
END;
$$ language 'plpgsql';
...
-- Enhanced: Trigger to automatically create profile after user creation
...
-- View for listings with all details including owner profile
